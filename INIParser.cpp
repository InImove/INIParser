...
int INIParser::ReadINI(char* str)
{
    ifstream in_conf_file;
    in_conf_file.open(str,ios::in);
    if(!in_conf_file.is_open())
    {
        qDebug("INIParser::ReadINI______in_conf_file open fail___");
        return 0;
    }

    if(in_conf_file.peek() == EOF)
    {
        qDebug("INIParser::ReadINI__in_conf_file en________");
        in_conf_file.close();
        in_conf_file.clear();
        system("cp /home/back/config.ini /home/");
        in_conf_file.open("/home/config.ini",ios::in);
    }

    system("cp /home/config.ini /home/back/");
    string str_line = "";
    string str_root = "";
...
string INIParser::GetValue(string key,string root)
{
...
{
    ofstream out_conf_file;//("/home/config.ini");
    out_conf_file.open("/home/config.ini",ios::out);
...
//set value
vector<ININode>::size_type INIParser::SetValue(string key, string value,string root)
{
    map<string, SubNode>::iterator itr = map_ini.find(root);
    if(map_ini.end() != itr)
